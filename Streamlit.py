...
with col1:
   ...
   X3= st.slider("Mn (wt. %)", 0, 20.0)
   X4 = st.slider("Si (wt. %)", 0, 1.5)
   X5 = st.slider("Co (wt. %)", 0, 0.04)
   X6= st.slider("Mo (wt. %)", 0, 2.95)
   X7 = st.slider("Ni (wt. %)", 0, 100.0)

with col2:
   X8=st.slider("C (wt. %)", 0, 1.0)
   X9 = st.slider("Ti (wt. %)", 0, 2.2)
   X10= st.slider("N (wt. %)", 0, 0.3)
   X11 = st.slider("B (wt. %)", 0, 0.004)
   X12 = st.slider("P (wt. %)", 0, 0.155)
   X13= st.slider("S (wt. %)", 0, 0.03)
   X14 = st.slider("Nb (wt. %)", 0, 0.92)   
	
with col3:
   X15=st.slider("Cu (wt. %)", 0, 0.54)
   X16 = st.slider("Ta (wt. %)", 0, 0.36)
   X17= st.slider("Al (wt. %)", 0, 100.0)
   X18 = st.slider("V (wt. %)", 0, 2.0)
   X19 = st.slider("Mg (wt. %)", 0, 1.63)
   X20= st.slider("W (wt. %)", 0, 2.4)
   X21 = st.slider("Zr (wt. %)", 0, 0.1)  

with col4:	
   X22=st.slider("Dose rate (dpa/s)", 0, 97.0)
   X23 = st.slider("Temperature (K)", 0, 24.7)
   X24= st.slider("Irradiation type", 1, 4)
   X25 = st.slider("Dislocation density x10^14 (m\u00B(−2))", 0.3, 38.5)
   X26 = st.slider("Pre-injected He (appm)", 0, 100.0)
